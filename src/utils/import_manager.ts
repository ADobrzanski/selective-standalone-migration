...
export class ImportManager {
  ...
  addImportToSourceFile(
    ...
    for (let i = sourceFile.statements.length - 1; i >= 0; i--) {
      ...
      if (
        (moduleSpecifier.startsWith(".") &&
          resolve(sourceDir, moduleSpecifier) !==
          ...
          const existingElement = namedBindings.elements.find((e) => {
            ...
            if (alias) {
              return (
                e.propertyName &&
                e.name.text === alias &&
                e.propertyName.text === symbolName
              );
            }
            return e.propertyName
              ? e.propertyName.text === symbolName
              : e.name.text === symbolName;
          });
    ...
    if (symbolName) {
      ...
      importMap
        .get(moduleName)!
        .push(ts.factory.createImportSpecifier(false, propertyName, name));
    } else {
  ...
  recordChanges() {
    ...
    this.newImports.forEach(
      ({ importStartIndex, defaultImports, namedImports }, sourceFile) => {
        const recorder = this.getUpdateRecorder(sourceFile);
        const useSingleQuotes =
          this._getQuoteStyle(sourceFile) === QuoteStyle.Single;
  ...
  private _getImportParts(
    ...
    const needsGeneratedUniqueName =
      generatedUniqueIdentifier.text !== (alias || symbolName);
    ...
    if (!this.quoteStyles.hasOwnProperty(sourceFile.fileName)) {
      ...
      for (const statement of sourceFile.statements) {
        if (
          ...
          quoteStyle = statement.moduleSpecifier
            .getText()
            .trim()
            .startsWith('"')
            ? QuoteStyle.Double
